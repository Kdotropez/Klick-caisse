...
console.log('🔧 Correction des catégories (version corrigée)...');
...
const newDataLines = dataLines.map(line => {
  ...
  if (parts.length >= 6) {
    ...
    // Déplacer CATEGORIE 1 vers Accueil (ID 4)
    if (category === '1') {
      parts[5] = '4'; // ID de Accueil
      modifiedLines++;
    ...
    // Supprimer CATEGORIE 2 et 3 (les produits ne seront plus dans ces catégories)
      ...
      parts[5] = '17'; // ID de DIVERS (catégorie par défaut)
      modifiedLines++;
...
// Supprimer les lignes 2 et 3 (CATEGORIE 2 et 3) et renommer CATEGORIE 1
