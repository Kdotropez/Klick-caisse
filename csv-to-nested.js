...
console.log('🔄 Conversion CSV → JSON NESTED...');
...
try {
    ...
    const nestedData = articles.map(article => {
        ...
        return {
            ...
            sousCategorie: article['sous categorie '] || '', // IMPORTANT: Sous-catégorie
    ...
    if (withSubcategories.length > 0) {
        ...
        withSubcategories.forEach(p => subcats.add(p.sousCategorie));
        Array.from(subcats).slice(0, 10).forEach(sub => {
            console.log(`   - ${sub}`);
        });
    }
    ...
    // Aperçu
    console.log(`\n🔍 Aperçu des produits:`);
    nestedData.slice(0, 3).forEach((p, i) => {
        console.log(`${i+1}. ${p.nom} (${p.categorie}) - ${p.prixTTC}€`);
        console.log(`   Sous-catégorie: ${p.sousCategorie || 'Aucune'}`);
