            ...
            <Box sx={{ p: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
             ...
             <Box sx={{ 
               ...
               {currentProducts.map((product) => (
                 ...
                 >
                                     <Typography variant="body2" sx={{ fontWeight: 'bold', fontSize: '0.75rem', lineHeight: 1.2, flexGrow: 1 }}>
                     {product.name}
                   </Typography>
                   <Typography variant="h6" color="primary" sx={{ fontWeight: 'bold', fontSize: '1rem', textAlign: 'center' }}>
                     {product.finalPrice.toFixed(2)} €
                   </Typography>
                   <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 0.25, gap: 0.25 }}>
                     <Chip label={`${product.salesCount || 0}`} size="small" sx={{ fontSize: '0.6rem', height: '20px' }} />
                     {product.variations.length > 0 && (
                       <Chip label={`${product.variations.length} var.`} size="small" sx={{ fontSize: '0.6rem', height: '20px' }} />
                     )}
                   </Box>
                </Paper>
              ))}
